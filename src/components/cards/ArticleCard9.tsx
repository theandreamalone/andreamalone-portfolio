...
export default function ArticleCard9({ card, idx }: CardProps) {
  return (
    <>
      <div className="article card-9 d-flex flex-md-row align-items-stretch flex-column" key={idx}>
        <Link href={card.linkPost} className="card-img-top">
          <Image src={card.img} alt="magzin" className="cover-image thumbnail" width={230} height={250} />
        </Link>
          ...
          <div className="left">
            <div className="card-info d-flex align-items-center mb-3">
              ...
              <ul className="d-flex align-items-center text-600 m-0 ps-4">
                <li>
                  <p className="fs-8 m-0">{card.read}</p>
                </li>
              </ul>
            </div>
            <Link href={card.linkPost}>
              <h6 className="card-title mb-0 mt-3 text-truncate-1">{card.title}</h6>
            </Link>
            <p className="card-text text-600 fs-7 mb-0 mt-3 text-truncate-2">{card.description}</p>
